...
cpdEvaluateUStar_manual_test_cases = [
    "2007",  # Nomial case
    "2007_fnight_zero_0",  # All nighttime data is zero
]
cpdEvaluateUStar_test_cases = (
    cpdEvaluateUStar_iteration_test_cases + cpdEvaluateUStar_manual_test_cases
)


@pytest.mark.parametrize("year", cpdEvaluateUStar_test_cases)
def test_cpdEvaluateUStarTh4Season20100901_logged_data(
    test_engine, year
):  # This test stores the logged data as row major instead of column major
    """
    ...
    [xCp2, xStats2, xCp3, xStats3] = test_engine.cpdEvaluateUStarTh4Season20100901(
        input_data["time_it_"],
        input_data["NEE_it_"],
        input_data["updated_uStar_it_"],
        input_data["Temperature_it_"],
        input_data["fNight_it_"],
        fplot,
        cSiteYr,
        jsonencode=[1, 3],
        nargout=4,
    )  # TODO: Change jsonencode explicity named argument
...
testcases = [
    ([1.0000, 1.0417, 1.0834], 3, [0, 1, 1], 366, 3, 2400),  # nPerDay = 24, nPerBin = 3
    ([1.083, 1.083, 1.125], 3, [1, 1, 1], 366, 5, 4000),  # nPerBin = 5
]


@pytest.mark.parametrize(
    "t, expected_nt, expected_m, expected_EndDOY, expected_nPerBin, expected_nN",
    testcases,
)
def test_initializeParameters(
    test_engine,
    t,
    expected_nt,
    expected_m,
    ...
    nt, m, EndDOY, nPerBin, nN = test_engine.initializeParameters(
        t, nSeasons, nStrataN, nBins, nargout=5
    )
...
def test_filterInvalidPoints_logged_data(test_engine):
    ...
    for name in expected_output_names:
        path_to_artifacts = (
            artifacts_dir + f"/CA-Cbo_qca_ustar_2007_0/output_{name}.csv"
        )
        column = pd.read_csv(path_to_artifacts, header=None).iloc[:, 0].to_numpy()
        if name == "itAnnual":
    ...
    assert test_engine.equal(
        test_engine.convert(itAnnual), expected_output_data["itAnnual"]
    )
    assert test_engine.equal(
        test_engine.convert(ntAnnual), expected_output_data["ntAnnual"]
    )
...
def test_initializeStatistics(test_engine):
    ...
    Stats2, Stats3 = test_engine.initializeStatistics(
        nSeasons, nStrataX, jsonencode=[0, 1], nargout=2
    )  # TODO: Change jsonencode explicity named argument
...
def test_reorderAndPreprocessData_logged_data(test_engine):
    ...
    t, T, uStar, NEE, fNight, itAnnual, ntAnnual = test_engine.reorderAndPreprocessData(
        *[input_data[name] for name in input_names], nargout=7
    )

    assert test_engine.equal(test_engine.convert(t), expected_output_data["time"])
    assert test_engine.equal(
        test_engine.convert(T), expected_output_data["Temperature"]
    )
    assert test_engine.equal(test_engine.convert(uStar), expected_output_data["uStar"])
    assert test_engine.equal(test_engine.convert(NEE), expected_output_data["NEE"])
    assert test_engine.equal(
        test_engine.convert(fNight), expected_output_data["fNight"]
    )
    assert test_engine.equal(
        test_engine.convert(itAnnual), expected_output_data["itAnnual"]
    )
    assert test_engine.equal(
        ntAnnual, test_engine.convert(expected_output_data["ntAnnual"])
    )
...
def test_computeStrataCount(test_engine, ntSeason, nPerBin, expected_nStrata):
    ...
    nStrata = test_engine.computeStrataCount(
        ntSeason, nBins, nPerBin, nStrataN, nStrataX
    )
...
def test_computeTemperatureThresholds_logged_data(test_engine):
    ...
    TTh = test_engine.computeTemperatureThresholds(
        test_engine.convert(input_data["T"]),
        test_engine.convert(input_data["itSeason"], "to_python"),
        nStrata,
        nargout=1,
    )
    python_TTh = computeTemperatureThresholds(
        np.array(input_data["T"]), input_data["itSeason"] - 1, nStrata
    )  # -1 to account for 0-based indexing in python
    ...
    assert test_engine.equal(
        test_engine.convert(TTh), test_engine.convert(expected_TTh)
    )
    assert test_engine.equal(
        test_engine.convert(python_TTh), test_engine.convert(expected_TTh)
    )
