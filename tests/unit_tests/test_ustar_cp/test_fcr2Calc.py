...
def float_list(draw, exclude_non_constant=True):
    ...
    xs = draw(
        lists(
            floats(allow_infinity=False, min_value=-1e9, max_value=1e9),
            min_size=1,
            max_size=4,
        )
    )
...
@given(list_data=same_len_float_lists(), scalar=floats(min_value=0.1, max_value=1000))
...
@given(
    data1=float_list(exclude_non_constant=True),
)
