...
def test_loadData(year, setup_folders, matlab_engine):
    """
    ...
    matlab_engine.loadData(input_folder, filename, notes, input_columns_names, output_folder, nargout=0)
...
def test_areAllPpfdValuesInvalid(matlab_engine, year_and_type, expected_ppfd_from_rg):
    """
    ...
    # Call the MATLAB function
    output_ppfd, output_ppfd_from_rg = matlab_engine.areAllPpfdValuesInvalid(ppfd_from_rg, columns_index, ppfd_index, data, site_data_file, nargout=2)
...
def test_derivePpfdColFromRg(matlab_engine, year):
    """
    ...
    # Call the MATLAB function
    output_ppfd = matlab_engine.derivePpfdColFromRg(rg, nargout=1)
...
@pytest.mark.parametrize("year", ['2005', '2006'])
def test_setMissingDataNan(matlab_engine, year):
    """
    ...
    matlab_engine.setMissingDataNan(site_columns['uStar'], site_columns['NEE'], site_columns['Ta'], site_columns['PPFD'], site_columns['Rg'], nargout=5)
...
@pytest.mark.parametrize("year", ['2005', '2006'])
def test_createTimeArray(matlab_engine, year):
    ...
    # Call the MATLAB function
    output_t = matlab_engine.createTimeArray(ustar, nargout=1)
