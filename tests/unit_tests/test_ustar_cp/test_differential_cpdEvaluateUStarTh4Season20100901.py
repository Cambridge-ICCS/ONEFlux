...
def json_to_numpy(data):
    ...
    for record_group in data:  # Handle nested lists in the JSON
        ...
        for record in record_group:
            array = np.array(
                list(record.values()), dtype=float
            )  # Convert to numpy array
            group_arrays.append(array)
        all_arrays.append(
            np.array(group_arrays)
        )  # Group into a 2D array for each group
...
def test_filterInvalidPoints_differential(test_engine, uStar, fNight, NEE, T):
    ...
    expected_u_star, expected_valid_annual_indices, expected_num_valid_annual = (
        filterInvalidPoints(uStar, fNight, NEE, T)
    )
    # 'to_matlab' optional argument to account for 1-based indexing in MATLAB
    expected_u_star, expected_valid_annual_indices, expected_num_valid_annual = (
        test_engine.convert(expected_u_star),
        test_engine.convert(expected_valid_annual_indices, "to_matlab"),
        test_engine.convert(expected_num_valid_annual),
    )
    ...
    uStar, itAnnual, ntAnnual = test_engine.filterInvalidPoints(
        uStar, fNight, NEE, T, nargout=3
    )

    assert test_engine.equal(test_engine.convert(uStar), expected_u_star)
    assert test_engine.equal(
        test_engine.convert(itAnnual), expected_valid_annual_indices
    )
...
testcases = [
    (
        rng.uniform(0, 5, size),
        rng.uniform(0, 5, size),
        rng.uniform(0, 5, size),
        rng.uniform(0, 5, size),
        np.random.randint(0, 2, size),
        366,
        np.repeat(np.arange(1, 13), occurrences),
        size,
    ),  # Nominal case
    (
        rng.uniform(0, 5, size),
...
def test_reorderAndPreprocessData_differential(
    test_engine, t, T, uStar, NEE, fNight, EndDOY, m, nt
):
    """
    ...
    t, T, uStar, NEE, fNight, itAnnual, ntAnnual = test_engine.reorderAndPreprocessData(
        *matlab_data, nargout=7
    )
    ...
    # 'to_matlab' optional argument to account for 1-based indexing in MATLAB
    assert test_engine.equal(
        test_engine.convert(itAnnual),
        test_engine.convert(expected_itAnnual, "to_matlab"),
    )  # +1 to match matlab indexings
...
testcases = [
    ...
    (
        ...
        4999,
        range(3751, 4999 + 1),
    ),  # iSeasons = 4, ntAnnual is odd
]


@pytest.mark.parametrize(
    "iSeasons, nSeasons, nPerSeason, ntAnnual, expected_jtSeasons", testcases
)
def test_computeSeasonIndices_differential(
    test_engine, iSeasons, nSeasons, nPerSeason, ntAnnual, expected_jtSeasons
):
    """
    ...
    jtSeasons = test_engine.computeSeasonIndices(
        test_engine.convert(iSeasons, "to_matlab"),
        nSeasons,
        nPerSeason,
        test_engine.convert(ntAnnual),
    )

    assert test_engine.equal(
        test_engine.convert(list(python_jtSeasons), "to_matlab"),
        test_engine.convert(list(jtSeasons)),
    )


tetcases = [
    (
        rng.uniform(-20, 20, 1000),
        np.array(rng.choice(1000, size=500, replace=False)),
        [-14.601, -5.22904, -2.095605, 1.836835, 5.136015, 14.2835],
        0,
    ),  # Nominal case
    (
        np.array([-20, -19, -15, 1, 4, 14, 8, 7, 8, 4], dtype=float),
        np.array([0, 1, 2]),
        [-14.601, -5.22904, -2.095605, 1.836835, 5.136015, 14.2835],
        1,
    ),  # # No matching indices
    (
        ...
        [1, 1, 1, 1, 1, 1],
        4,
    ),  # Non increasing threshold TTH
    (
        rng.uniform(-5, 25, 900),
        np.array(rng.choice(1000, size=300, replace=False)),
        [-1.34166, 5.217795, 8.340625, 12.45965, 16.9017, 24.672],
        2,
    ),  # iStrata = 2
    (
        rng.uniform(0, 30, 400),
        np.array(rng.choice(1000, size=200, replace=False)),
        [2.34335, 6.89097, 11.09125, 14.6745, 17.95015, 27.262],
        4,
    ),  # iStrata = 4
    (
        rng.uniform(-5, 15, 800),
        np.array(rng.choice(1000, size=400, replace=False)),
        [-2.22904, 0.904395, 4.836835, 8.136015, 13.2835],
        3,
    ),  # iStrata = 3, length(TTh) = 5
]
...
def test_findStratumIndices_differential(test_engine, T, itSeason, TTh, iStrata):
    ...
    T = test_engine.convert(T)
    # 'to_matlab' optional argument to account for 1-based indexing in MATLAB
    itSeason = test_engine.convert(
        np.array(itSeason, dtype=float), "to_matlab"
    )  # +1 to account for 1-based indexing in MATLAB
    TTh = test_engine.convert(TTh)
    iStrata = test_engine.convert(
        iStrata, "to_matlab"
    )  # +1 to account for 1-based indexing in MATLAB

    itStrata = test_engine.findStratumIndices(T, itSeason, TTh, iStrata, nargout=1)
    assert test_engine.equal(
        itStrata, test_engine.convert(expected_itStrata, "to_matlab")
    )  # +1 to account for 1-based indexing in matlab
...
def test_addStatisticsFields_differential(test_engine):
    ...
    # 'to_matlab' optional argument to account for 1-based indexing in MATLAB
    xs = test_engine.addStatisticsFields(
        xs,
        test_engine.convert(t),
        test_engine.convert(r),
        test_engine.convert(p),
        test_engine.convert(T),
        test_engine.convert(itStrata, "to_matlab"),
        nargout=1,
    )
