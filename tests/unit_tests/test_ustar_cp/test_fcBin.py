...
def avoidOverflows(data, maxFloatSize=1e8):
    ...
    """
    if np.all(
        [np.abs(item) <= maxFloatSize or np.isnan(item) for item in np.array(data).flat]
    ):
        return data
    else:
...
@given(
    dataIn=lists(floats(allow_nan=True, allow_infinity=False), min_size=2),
...
def test_singleton_bins_1D_data(dataIn, scale, translate, test_engine):
    ...
    nBins, mx, my = test_engine.fcBin(
        test_engine.convert(data1),
        test_engine.convert(data2),
        test_engine.convert([]),
        1.0,
        nargout=3,
    )
        ...
        if isinstance(ys, float):
            ...
            for bin in ys:
                ...
                if not (np.isinf(bin[0])):
                    assert np.any(
                        [np.isclose(bin[0], item, equal_nan=True) for item in data]
                    )
...
def test_singleton_bins_2D_data(dataIn, scale, row, translate, test_engine):
    ...
    # Use the initial data to generate two vectors worth of data
    # based on some scaling and translation to get data2
    data2 = avoidOverflows(
        [[scale * item + translate for item in row] for row in data1]
    )
...
test_data_dx_length_eq_zero = [
    # Cases with empty dx
    {"x": [], "y": [], "dx": [], "nPerBin": 1.0, "mx": [], "my": [], "nBins": 0},
    {
